...
class certificate
{
	...
	function get_enquiry_by_user($user){
		...
		$sql = sprintf("select distinct(enquiry_id) from history where created_by='$user' and isactive=1"); 
		            $rs=mysql_query($sql,$this->link);
		            if(!$rs){
			         echo mysql_error($this->link);
			         }else{
	...
	function update_in_report($enq_id,$dates){ //date of certification issue
	
	echo $sql = sprintf("update reports_of_enquiry set d_o_scop_cert_issue='$dates'  where enq_id='$enq_id' and isactive=1"); 
		            $rs=mysql_query($sql,$this->link);
		            if(!$rs){
			         echo mysql_error($this->link);
			         }else{
				...
				function update_in_report_for_docd($enq_id,$dates,$cert_status){//date of certification decision
	
	echo $sql = sprintf("update reports_of_enquiry set d_o_cert_decision='$dates', certification_status='$cert_status'  where enq_id='$enq_id' and isactive=1"); 
		            $rs=mysql_query($sql,$this->link);
		            if(!$rs){
			         echo mysql_error($this->link);
			         }else{
...
function insert_enq_per($table,$column,$enq_id,$fmid,$fmname,$company,$pos,$street,$taluka,$district,$stat,$country,$pincode,$mobile,$phno,$email,$location,$ics){
	...
	function update_status($enq_id,$status,$remark,$cert_status){
	
	$sql = sprintf("update enquiry_status set current_status='$status',cert_status_after_decision='$cert_status',remarks='$remark', is_seen=1 where enquiry_id='$enq_id' and isactive=1"); 
		            $rs=mysql_query($sql,$this->link);
		            if(!$rs){
			         echo mysql_error($this->link);
			         }else{
